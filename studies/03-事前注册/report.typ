#set document(title: "事前注册——先写规则，再看数据")
...
- *演示一（随机世界）*：在 400 个**完全无信号**的世界里扫描 16 档阈值——*40% 的「发现」能过 t>2*；同一规则样本外 t 均值归零（-0.09），*50% 变负、49% 连触发机会都没有*；
- *演示二（真实数据）*：同一套流程，不同年代"发现"不同阈值（*30 → 32 → 46*，贴着当时历史水位）；样本内 **+126%** 的"最优"规则（2024 年选出的阈值 46），前向只剩 *+13%*；前向 5 年大多只触发 0–1 次；
...
在 400 个**完全没有信号**的随机世界里，模拟同一套研究流程：扫描 16 档阈值，挑表现最好的那一档（t 检验 vs 基准；样本内触发的未来收益也必须在样本内"结算"）。

- 样本内"最优阈值"的 t 均值 = *1.73*，*P(t>2) = 40%*——四成的世界里，"研究"能找到"统计显著"的规律，而那里本来什么都没有；
- 把**同一个规则**放到样本外：t 均值 = *-0.09*；*50% 变负*；*49% 连触发机会都没有*；P(t>2) 降到 14%。
...
- 最刺眼的一组：2024 年选出的最优规则（阈值 46）——样本内 *+126%*，前向 *+13%*。
...
#text(size: 8.5pt)[#table(columns: 6, stroke: 0.3pt + rgb("#dddddd"), inset: 3.5pt,
  ...
  [2008], [30], [+26%（n=4）], [0], [—], [0],
  [2010], [30], [+27%（n=4）], [0], [—], [0],
  [2012], [30], [+26%（n=4）], [1], [+63%], [0],
  [2014], [30], [+28%（n=4）], [1], [+60%], [0],
  [2016], [30], [+31%（n=4）], [1], [+75%], [2],
  [2018], [32], [+46%（n=5）], [1], [+84%], [2],
  [2020], [32], [+48%（n=5）], [2], [+11%], [1],
  [2022], [46], [+124%（n=5）], [2], [+41%], [2],
  [2024], [46], [+126%（n=5）], [2], [+13%], [2],
)]

== 阈值稀疏度（全历史）

#table(columns: 3, stroke: 0.4pt + rgb("#cccccc"), inset: 5pt,
...
*首张实卡（2026-09-17，已生成待签）* —— `cards/注册卡-2026-09-17-GOR.md`
...
- 注册卡：`python scripts/make_card.py` → cards/注册卡-2026-09-17-GOR.md
